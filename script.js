(() => {
  ...
  const navItems = [
    ...
    ["transparencia.html", "Transparência"], ["noticias.html", "Histórias"],
  ...
  const schema = document.createElement("script"); schema.type = "application/ld+json"; schema.textContent = JSON.stringify({ "@context": "https://schema.org", "@type": "NGO", name: org.publicName, legalName: org.legalName, url: urls.canonical, email: org.email, telephone: org.whatsappDisplay, taxID: org.cnpj, foundingDate: String(org.foundedYear), address: { "@type": "PostalAddress", streetAddress: "Rua Alair Pereira da Silva, 205", addressLocality: "Belo Horizonte", addressRegion: "MG", addressCountry: "BR" }, sameAs: [org.instagram] }); document.head.appendChild(schema);

  const footer = document.querySelector("footer.footer");
  if (footer) footer.innerHTML = `<div class="footer-main"><div><img src="assets/images/logo-rede-asas-principal-crop.jpeg" alt="Rede ASAS Brasil" width="760" height="430"><p>Desde 1996, educação, cuidado e oportunidades.</p></div><div><h3>Institucional</h3><a href="quem-somos.html">Quem somos</a><a href="projetos.html">Projetos</a><a href="impacto.html">Impacto</a><a href="transparencia.html">Transparência</a></div><div><h3>Participe</h3><a href="novo-predio.html">Novo prédio</a><a href="apoie.html">Quero apoiar</a><a href="noticias.html">Histórias</a><a href="privacidade.html">Privacidade</a></div><div><h3>Contato oficial</h3><a href="mailto:${org.email}">${org.email}</a><a href="https://wa.me/${org.whatsappNumber}">${org.whatsappDisplay}</a><p>${org.address}</p></div></div><div class="footer-legal"><span>© ${new Date().getFullYear()} ${org.publicName}</span><span>CNPJ ${org.cnpj}</span></div>`;
  ...
  const closeMenu = () => { nav?.classList.remove("is-open"); menuToggle?.setAttribute("aria-expanded", "false"); menuToggle?.setAttribute("aria-label", "Abrir menu"); };
  ...
  document.querySelectorAll("[data-lead-form],[data-contact-form]").forEach((form) => form.addEventListener("submit", (event) => {
    ...
    if (status) status.textContent = "Seu aplicativo de e-mail será aberto para concluir o envio. Para resposta imediata, use o WhatsApp oficial.";
  }));

  const helpWidget = document.createElement("aside");
  helpWidget.className = "help-widget";
  helpWidget.innerHTML = `<button class="help-trigger" type="button" aria-expanded="false" aria-controls="help-panel"><span class="help-dot" aria-hidden="true"></span>Posso ajudar?</button><div class="help-panel" id="help-panel" hidden role="dialog" aria-modal="false" aria-labelledby="help-title"><button class="help-close" type="button" aria-label="Fechar atendimento">×</button><p class="help-greeting">Olá! 👋</p><h2 id="help-title">Como podemos ajudar?</h2><p>Escolha uma opção para falar com a equipe.</p><div class="help-options">${[["fazer uma doação", "Quero fazer uma doação"], ["ajudar na construção do novo prédio", "Apoiar o novo prédio"], ["conversar sobre uma parceria empresarial", "Represento uma empresa"], ["ser voluntário(a)", "Quero ser voluntário(a)"], ["agendar uma visita institucional", "Agendar uma visita"]].map(([message,label]) => `<a href="https://wa.me/${org.whatsappNumber}?text=${encodeURIComponent(`Olá, quero ${message} na Rede ASAS.`)}">${label}<span>→</span></a>`).join("")}</div><small>WhatsApp oficial: ${org.whatsappDisplay}</small></div>`;
  ...
  const setHelp = (open) => { panel.hidden = !open; trigger.setAttribute("aria-expanded", String(open)); if (open) closer.focus(); };
