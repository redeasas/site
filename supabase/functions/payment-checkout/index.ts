...
Deno.serve(async request => {
  ...
  const checkoutResponse = await fetch(`${baseUrl}/v3/checkouts`,{method:"POST",headers:{"Content-Type":"application/json","access_token":apiKey},body:JSON.stringify({billingTypes:["CREDIT_CARD"],chargeTypes:["RECURRENT"],minutesToExpire:60,externalReference:agreement.id,callback:{successUrl:"https://redeasas.org.br/1000-asas?pagamento=sucesso",cancelUrl:"https://redeasas.org.br/1000-asas?pagamento=cancelado",expiredUrl:"https://redeasas.org.br/1000-asas?pagamento=expirado"},items:[{name:"Campanha 1.000 ASAS",description:"Contribuição mensal para continuidade dos projetos sociais",quantity:1,value:amount}],subscription:{cycle:"MONTHLY",nextDueDate:`${nextDue.toISOString().slice(0,10)} 12:00:00`}})});
