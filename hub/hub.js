(async () => {
  ...
  if (!authContext.demo && ["mantenedor","financeiro","empresas","voluntarios","impacto","ia"].includes(view)) {
    const modules = {
      ...
      financeiro: { table:"asas_payment_events", select:"occurred_at,status,amount,fee_amount,net_amount,billing_type", order:"occurred_at.desc", columns:["Data","Status","Bruto","Tarifa","Líquido","Forma"], values:r=>[new Date(r.occurred_at).toLocaleDateString("pt-BR"),r.status,r.amount==null?"—":Number(r.amount).toLocaleString("pt-BR",{style:"currency",currency:"BRL"}),r.fee_amount==null?"—":Number(r.fee_amount).toLocaleString("pt-BR",{style:"currency",currency:"BRL"}),r.net_amount==null?"—":Number(r.net_amount).toLocaleString("pt-BR",{style:"currency",currency:"BRL"}),r.billing_type||"—"] },
